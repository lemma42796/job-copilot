...
对照 scoring_points 列表,逐 point 判断用户答案是否覆盖:
...
每条 claim 必须配 supporting_chunk_ids:支持该声明的 chunks 编号([N] 标号)。fabricated 的 supporting_chunk_ids 留空数组。
...
5. user_excerpt / supporting_chunk_ids 必须是真实存在的引用,不要编造
6. coach_message 是给用户看的教练反馈,必须基于本次 evidence:
   - 先用自然中文评价这轮答案已经做对的地方
   - 再指出最重要的不足,优先覆盖缺口 / fabricated 风险 / 深度缺失
   - 最后给下一轮补答方向;如果没有明显缺口,说明这题可以收住,等待系统进入下一步
   - 不要编造 scoring_points / chunks 外的新标准答案,不要输出分数公式
...

{
  "coverage_evidence": {
    "points": [
      {"id": "<scoring_point.id>", "label": "hit"|"partial"|"miss", "user_excerpt": "<...>"|null}
    ],
    "score_raw": <float 0-1>,
    "reasoning": "<中文一句话>"
  },
  "fidelity_evidence": {
    "claims": [
      {"text": "<用户原文片段>", "label": "supported"|"inferred"|"fabricated", "supporting_chunk_ids": [<int>, ...]}
    ],
    "score_raw": <float 0-1>,
    "reasoning": "<中文一句话>"
  },
  ...
  "coach_message": "<中文自然反馈,2-4 句>"
}

注:claims 里的 supporting_chunk_ids 默认写上下文 chunks 的 [N] 标号;若采用工具结果作为证据,
...
工具结果作为证据,claims[].supporting_chunk_ids 写工具返回的 ref_id,不要写 chunk_id。"""
...
def render_user(
    *,
    ...
    """
    points = []
    for p in question.scoring_points:
        evidence_ids = ", ".join(f"[{n}]" for n in p.supporting_chunk_ids)
        points.append(
