...
NoteChunk ORM 实体放进 FinalContextChunk.chunk(不复制字段),quiz_service
渲染 USER 段时拿 chunk.id / chunk.content;heading_path 反规范化字段从
NoteChunk 取也行,但 retrieval_pipeline 落 FinalContextChunk 时显式复制一份
...
class FinalContextChunk:
    """最终喂给 quiz_generator / Judge 的单个上下文 chunk。"""
...
class PipelineResult:
    ...
    expanded_queries / final_context_chunks 由 quiz_service 一并落 quiz_sessions
    的 expanded_queries / final_context_chunk_ids 字段(DATA_MODEL §5.4 audit)。
    """

    expanded_queries: list[str]
    final_context_chunks: list[FinalContextChunk]
