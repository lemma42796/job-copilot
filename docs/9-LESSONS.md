...
- **最终决策**:停止把 `candidate_recall@50` 当主胜利指标,但保留 top50 作为 provider rerank input 和诊断窗口。当前生产路径保留四层治理:source/type governance、极窄 protected anchor route、zero-hit support gate、post-rerank governance/blend + dynamic clean-context selection。provider rerank 是 challenger source,不是最终成员裁判;final selected 不为凑满 top10 塞低置信 chunk。
- **可迁移教训**:RAG 不是"召回越多越好",而是"正确证据要能进入候选池,最终上下文又必须干净"。粗排 top15 看紧窗口质量,top50 看长尾召回和 reranker 可救空间;当 top50 有、top10 没有时,问题是排序和候选治理;当 top50 本身漏时,才回到 query rewrite / tokenization / chunker / embedding。补召回和精排都必须有治理层兜底,不能让它们绕过 source/type、contrast、hard-negative 规则。
- **面试讲法**:我遇到一个 RAG 质量卡点,表面看扩大 topK 可以提高 recall,但产品真正吃的是一个很小、很干净的上下文窗口。我先搭 chunk-level eval 和 trace,拆出"粗召回、粗排排序、精排、hard-negative、zero-hit"五类责任。source/type governance 把 top10 recall 从 54.17% 提到 64.17%;收窄强锚点后到 71.67%;裸 provider rerank 会掉到 69.17% 且带回 hard-negative。最后我把 provider 改成 challenger,再用 post-rerank governance/blend 做最终成员裁决,初版 smoke 到 12/12、selected recall 86.67%、hard-negative 0/12。代价是系统多了一层可解释规则,但换来的是下游出题 / 评分不被泛相关 chunk 污染。
